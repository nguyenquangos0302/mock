...
async function getBreadCrumb(pathData) {
  ...
  const rawResponse = await fetch(
    "http://localhost:8080/CourseOnline/client/api/v1/breadcrumb",
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ url: `/course${pathUrl}` }),
    }
  );
...
const contentByArticle = async (data) => {
  const rawResponse = await fetch(
    "http://localhost:8080/CourseOnline/client/api/v1/post",
    {
      method: "POST",
      headers: {
        Accept: "application/json",
...
const templatePost = (datas) => {
  ...
  datas.forEach((data) => {
    teamplate += `<div class="card card__block">
...
async function getDataArticle() {
  const articleResponse = await fetch(
    `http://localhost:8080/CourseOnline/client/api/v1${pathUrl}`
  );
