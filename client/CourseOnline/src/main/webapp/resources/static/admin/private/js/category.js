    ...
    data.forEach(element => {
        ...
        template += `<div style="margin-bottom: 10px">
                        ...
                        <button type="button" class="btn btn-primary"
                            ...
                            <i class="${element.icon}" style="margin-right: 10px;"></i>${element.name}</button>

                            <span class="_status" style="margin-top: -8px; background-color: ${element.status === 1 ? "#16a085" : "#c0392b"}; width:15px;
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="staticBackdropLabel-${index}">${element.name}</h5>
        ...
        template += `<div class="col-md-6">
                        ...
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-primary"><i class="${element.icon}" style="margin-right: 10px;"></i>${element.name}</h6>
                            </div>
                            <div class="card-body">
                                Topic: 
                                <div class="d-flex p-2 justify-content-between">
                                    ${element.children.length <= 0 ? "<div class='alert alert-primary'>No topic</div>" : renderTopic(element.children)}
                                </div>
                            </div>
                        </div>
                    </div>`;
...
async function renderParentOpiton() {
    ...
    const parentResponse = await fetch("http://localhost:8080/CourseOnline/client/api/v1/menu/parentgreaterthanone");
...
async function renderMenuAdmin() {
    const menuResponse = await fetch("http://localhost:8080/CourseOnline/client/api/v1/menu/allparentgreaterthanone");
...
createMenu.addEventListener('click', async function (event) {
    ...
    const rawResponse = await fetch("http://localhost:8080/CourseOnline/client/api/v1/menu/addition",
    {
        method: 'POST',
        headers: {
